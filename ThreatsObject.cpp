...
void ThreatsObject::CheckToMap(Map& gMap)
{
    ...
    else if(x_pos_ + width_frame_ > gMap._max_x)
    {
        x_pos_ = gMap._max_x - width_frame_ - 1;
    }
    if(y_pos_ > gMap._max_y)
    {
        come_back_time_ = 5;
    }
}
    ...
    else 
    {
        if(on_ground_ == true)
        {
            if(x_pos_ > animation_b_)
            {
                ...
                LoadImg("Pictu//threat_left.png", screen);
            }
            else if(x_pos_ < animation_a_)
            {
                input_type_.right_ = 1;
                input_type_.left_ = 0;
                LoadImg("Pictu//threat_right.png", screen);
            }
        }
        else
        {
